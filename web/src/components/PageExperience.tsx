...
export default function PageExperience({
  ...
  useEffect(() => {
    ...
    gsapCtxRef.current = gsap.context(() => {
      ...
      // Scene 2: light bg fades to depth #242F2A at 50% scroll
  ...
  function renderServiceDetail(svc: Service, svcIdx: number) {
            ...
            <div className="flex min-h-full flex-col px-10 py-14 lg:px-16">
                ...
                {imgSrc && (
                  <div className="lg:w-[40%] lg:flex-shrink-0">
                    ...
                    <div className="relative cursor-pointer overflow-hidden" style={{ border: "2px solid rgba(74,83,67,0.3)", aspectRatio: "4/3", boxShadow: "0 0 40px rgba(74,83,67,0.08), inset 0 0 20px rgba(36,47,42,0.3)" }} onClick={() => imgSrc && setLightboxSrc(imgSrc)}>
                      ...
                      {/* HUD bottom info */}
                      <div className="absolute bottom-0 left-0 right-0 px-3 py-2" style={{ background: "rgba(36,47,42,0.3)", borderTop: "1px solid rgba(74,83,67,0.15)" }}>
                        <span className="font-mono text-[8px] text-[#4A5343]/50">DEPTH — · VIS HD · GPS –</span>
                      </div>
                    </div>
                    {/* Monitor label */}
                    <div className="mt-2 text-center font-mono text-[9px] tracking-[0.4em] text-[#4A5343]/30 uppercase">
              ...
              <div className="mb-10">
                <div className="mb-4 flex items-center gap-4">
                  <span className="font-mono text-[9px] tracking-[0.5em] text-[#4A5343]/50 uppercase">
                    {lang === "bg" ? "Обхват на мисията" : "Mission scope"}
                  </span>
                  <div className="h-px flex-1 bg-[#4A5343]/10" />
                </div>
                <div className="grid gap-0 sm:grid-cols-2">
                  {meta.activities.map((act, i) => (
                    <div
                      ...
                      style={{ borderBottom: "1px solid rgba(74,83,67,0.08)", background: i % 2 === 0 ? "rgba(74,83,67,0.02)" : "transparent" }}
                    >
                      <span className="flex-shrink-0 font-mono text-[11px] text-[#4A5343]/35">{String(i + 1).padStart(2, "0")}</span>
                      <span className="text-sm leading-snug text-[#1A221E]/75">{act}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {galleryImages.length > 0 && (
                <div className="flex gap-2 pb-24">
                  {galleryImages.map((src, i) => (
                    <div key={i} className="sd-row relative h-32 flex-1 cursor-pointer overflow-hidden" style={{ border: "1px solid rgba(74,83,67,0.15)" }} onClick={() => setLightboxSrc(src)}>
      ...
      return (
        <div className="animate-bsdc-panel-in fixed inset-0 z-[500] flex flex-col overflow-hidden pointer-events-auto" style={{ background: "#8A9A86" }}>
          ...
          <div className="relative flex-shrink-0 overflow-hidden" style={{ height: "34vh" }}>
            ...
            <div className="pointer-events-none absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(74,83,67,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(74,83,67,0.06) 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
              ...
              {[80, 160, 240, 320].map((r, i) => (
                <div
                  ...
                  style={{
                    width: r * 2,
                    height: r * 2,
                    border: `1px solid rgba(74,83,67,${0.14 - i * 0.03})`,
                  }}
                />
              ))}
            </div>
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 20%, #8A9A86 100%)" }} />
            <div className="absolute bottom-0 left-0 right-0 px-10 pb-8 lg:px-16">
              <div className="sd-eyebrow mb-2 flex items-center gap-3">
                ...
                <span className="font-mono text-[10px] tracking-[0.4em] text-[#4A5343]">
                  {lang === "bg" ? "ХИДРОГРАФСКИ ОТДЕЛ" : "HYDROGRAPHIC DIVISION"}
                </span>
              </div>
              <h2 className="sd-title text-4xl font-light leading-none text-[#1A221E] lg:text-5xl">{svc.title}</h2>
            </div>
          </div>
            ...
            <div className="px-10 pt-8 pb-20 lg:px-16">
              <div className="flex flex-col gap-10 lg:flex-row lg:gap-16">
                ...
                <div className="lg:w-[45%]">
                  {svc.content && (
                    <div
                      className="sd-desc mb-8 text-sm leading-[1.9] text-[#1A221E]/55 [&_p]:mb-4"
                      dangerouslySetInnerHTML={{ __html: svc.content }}
                    />
                  )}
                  <table className="w-full text-sm">
                    <tbody>
                      {meta.cards.map((card, i) => (
                        <tr key={i} className="sd-stat" style={{ borderBottom: "1px solid rgba(74,83,67,0.1)" }}>
                          <td className="py-3 font-light text-[#1A221E]/70 w-1/2">{card.title}</td>
                          <td className="py-3 font-mono font-light text-right" style={{ color: "#4A5343" }}>{card.value}</td>
                          <td className="py-3 pl-3 text-xs text-[#1A221E]/40">{card.sub}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex-1">
                  <div className="mb-4 flex items-center gap-3">
                    <span className="font-mono text-[10px] tracking-[0.4em] text-[#4A5343]/60 uppercase">
                      {lang === "bg" ? "Обхват на услугата" : "Service scope"}
                    </span>
                    <div className="h-px flex-1 bg-[#4A5343]/15" />
                  </div>
                  <div className="flex flex-col gap-1">
                    {meta.activities.map((act, i) => (
                      <div key={i} className="sd-row flex gap-5 py-3.5" style={{ borderBottom: "1px solid rgba(74,83,67,0.08)" }}>
                        <div className="w-6 flex-shrink-0 font-mono text-xs text-[#4A5343]/35 pt-0.5">{String(i + 1).padStart(2, "0")}</div>
                        <div className="text-sm leading-snug text-[#1A221E]/75">{act}</div>
                      </div>
                    ))}
                  </div>
                  {galleryImages.length > 0 && (
                    <div className="mt-6 flex gap-2">
                      {galleryImages.map((src, i) => (
                        <div key={i} className="sd-row relative h-36 flex-1 cursor-pointer overflow-hidden" style={{ border: "1px solid rgba(74,83,67,0.15)" }} onClick={() => setLightboxSrc(src)}>
    ...
    if (svcIdx === 3) {
      return (
        <div className="animate-bsdc-panel-in fixed inset-0 z-[500] overflow-hidden pointer-events-auto" style={{ background: "#8A9A86" }}>
          {CloseBtn}{BackBtn}
          ...
          <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 60% 40%, rgba(74,83,67,0.12) 0%, transparent 60%), linear-gradient(to bottom, rgba(36,47,42,0.5) 0%, rgba(36,47,42,0.15) 50%, rgba(36,47,42,0.55) 100%)" }} />
          ...
          <div className="absolute inset-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            <div className="flex min-h-full flex-col justify-center px-8 py-14 lg:px-16">
              ...
              <h2 className="sd-title mb-10 max-w-2xl text-5xl font-light leading-[1.0] tracking-tight text-[#1A221E] lg:text-6xl">
                {svc.title}
              </h2>

              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-10">
                {meta.activities.map((act, i) => (
                  <div key={i} className="sd-card p-5 backdrop-blur-sm" style={{ background: "rgba(74,83,67,0.04)", border: "1px solid rgba(74,83,67,0.09)" }}>
                    <div className="mb-3 font-mono text-xs" style={{ color: "#4A5343" }}>
                      {String(i + 1).padStart(2, "0")}
                    </div>
                    <p className="text-sm leading-snug text-[#1A221E]/85">{act}</p>
                  </div>
                ))}
              </div>
              ...
              {svc.content && (
                <div
                  className="sd-desc mb-8 p-7 text-sm leading-[1.85] text-[#1A221E]/75 backdrop-blur-sm [&_p]:mb-3"
                  style={{ background: "rgba(36,47,42,0.15)", border: "1px solid rgba(74,83,67,0.06)", maxWidth: "52rem" }}
                  dangerouslySetInnerHTML={{ __html: svc.content }}
                />
              )}
              ...
              <div className="flex flex-wrap gap-4 mb-10">
                {meta.cards.map((card, i) => (
                  <div key={i} className="sd-stat flex-1 min-w-[140px] p-5 backdrop-blur-sm" style={{ background: "rgba(74,83,67,0.07)", border: "1px solid rgba(74,83,67,0.22)" }}>
                    <div className="font-mono text-3xl font-light text-[#1A221E]">{card.value}</div>
                    <div className="mt-1 text-[11px] uppercase tracking-wider" style={{ color: "#4A5343" }}>{card.title}</div>
                    <div className="text-[10px] text-[#1A221E]/40">{card.sub}</div>
                  </div>
                ))}
              </div>
              ...
              {galleryImages.length > 0 && (
                <div className="flex gap-3 pb-24">
                  {galleryImages.map((src, i) => (
                    <div key={i} className="sd-row relative h-44 flex-1 cursor-pointer overflow-hidden backdrop-blur-sm" style={{ border: "1px solid rgba(74,83,67,0.07)" }} onClick={() => setLightboxSrc(src)}>
    ...
    if (svcIdx === 4) {
      return (
        <div className="animate-bsdc-panel-in fixed inset-0 z-[500] flex flex-col overflow-hidden pointer-events-auto" style={{ background: "#8A9A86" }}>
          ...
          <div className="pointer-events-none absolute inset-0" style={{ backgroundImage: "radial-gradient(rgba(74,83,67,0.08) 1px, transparent 1px)", backgroundSize: "28px 28px" }} />
          <div className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 50%, transparent 30%, rgba(36,47,42,0.3) 100%)" }} />
          ...
          <div className="relative flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            <div className="px-10 pt-14 pb-24 lg:px-16">
              ...
              <div className="flex flex-col gap-8 mb-12 lg:flex-row lg:items-start lg:gap-12">
                <div className="lg:flex-1">
                  <div className="sd-eyebrow mb-3 flex items-center gap-3">
                    <div className="h-px w-8 bg-[#4A5343]" />
                    <span className="font-mono text-[10px] tracking-[0.45em] text-[#4A5343] uppercase">
                      {lang === "bg" ? "Строителство · СМР" : "Civil & Maritime Works"}
                    </span>
                  </div>
                  <h2 className="sd-title mb-5 text-4xl font-light leading-[1.0] text-[#1A221E] lg:text-5xl">{svc.title}</h2>
                  {svc.content && (
                    <div
                      className="sd-desc text-sm leading-[1.85] text-[#1A221E]/55 [&_p]:mb-3"
                      dangerouslySetInnerHTML={{ __html: svc.content }}
                    />
                  )}
                </div>
                {imgSrc && (
                  <div className="relative h-64 w-full overflow-hidden lg:h-72 lg:w-[38%] lg:flex-shrink-0" style={{ transform: "rotate(-1.5deg)", border: "1px solid rgba(74,83,67,0.2)", boxShadow: "8px 8px 0 rgba(36,47,42,0.25)" }}>
                    <img src={imgSrc} alt={svc.title} className="sd-image h-full w-full object-cover" style={{ filter: "brightness(0.75) saturate(0.7)" }} />
                    <div className="absolute bottom-0 left-0 right-0 px-3 py-2" style={{ background: "rgba(36,47,42,0.6)" }}>
                      <span className="font-mono text-[9px] tracking-widest text-[#1A221E]/70 uppercase">
              ...
              <div className="mb-10">
                <div className="mb-5 flex items-center gap-4">
                  <span className="font-mono text-[10px] tracking-[0.4em] text-[#4A5343] uppercase">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {meta.activities.map((act, i) => (
                    <div key={i} className="sd-card flex items-start gap-4 px-5 py-4" style={{ background: "rgba(74,83,67,0.04)", border: "1px solid rgba(74,83,67,0.08)", borderLeft: "3px solid rgba(74,83,67,0.35)" }}>
                      <span className="flex-shrink-0 font-mono text-[11px] text-[#1A221E]/40 w-7">{String(i + 1).padStart(2, "0")}</span>
                      <span className="text-sm leading-snug text-[#1A221E]/75">{act}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="mb-10 flex flex-wrap gap-0">
                {meta.cards.map((card, i) => (
                  <div key={i} className="sd-stat flex-1 min-w-[150px] px-6 py-5" style={{ background: "rgba(74,83,67,0.04)", border: "1px solid rgba(74,83,67,0.08)", borderRight: i < meta.cards.length - 1 ? "none" : undefined }}>
                    <div className="font-mono text-3xl font-light text-[#1A221E]">{card.value}</div>
                    <div className="mt-1 text-xs uppercase tracking-wider text-[#1A221E]/70">{card.title}</div>
                    <div className="text-[10px] text-[#1A221E]/30">{card.sub}</div>
                  </div>
                ))}
              </div>
              ...
              {galleryImages.length > 0 && (
                <div className="flex gap-2">
                  {galleryImages.map((src, i) => (
                    <div key={i} className="sd-row relative h-40 flex-1 cursor-pointer overflow-hidden" style={{ border: "1px solid rgba(74,83,67,0.15)" }} onClick={() => setLightboxSrc(src)}>
      ...
      return (
        <div className="animate-bsdc-panel-in fixed inset-0 z-[500] flex flex-col overflow-hidden pointer-events-auto" style={{ background: "#8A9A86" }}>
          ...
          <div className="relative flex-shrink-0 overflow-hidden" style={{ height: "38vh", minHeight: 240 }}>
            ...
            <div className="absolute inset-0" style={{ background: "linear-gradient(to top, #8A9A86 0%, rgba(36,47,42,0.4) 40%, rgba(36,47,42,0.15) 70%, transparent 100%)" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to right, rgba(36,47,42,0.6) 0%, transparent 50%)" }} />
            ...
            <div className="absolute bottom-0 left-0 right-0 px-10 pb-10 lg:px-16">
              ...
              <h2 className="sd-title mb-2 text-4xl font-light leading-[1.0] text-[#1A221E] lg:text-6xl">{svc.title}</h2>
              {svc.shortDescription && (
                <p className="max-w-xl text-base text-[#1A221E]/70">{svc.shortDescription}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            <div className="px-10 pt-10 pb-24 lg:px-16">
              {svc.content && (
                <div
                  className="sd-desc mb-10 max-w-3xl text-sm leading-[1.9] text-[#1A221E]/55 [&_p]:mb-3"
                  dangerouslySetInnerHTML={{ __html: svc.content }}
                />
              )}
              <div className="mb-8">
                <div className="mb-5 flex items-center gap-4">
                  <span className="text-[10px] uppercase tracking-[0.4em] text-[#4A5343]/70">
                    {lang === "bg" ? "Модули на обучението" : "Training modules"}
                  </span>
                  <div className="h-px flex-1" style={{ background: "rgba(74,83,67,0.2)" }} />
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                {meta.cards.map((card, i) => (
                  <div key={i} className="sd-stat p-5" style={{ background: "rgba(74,83,67,0.06)", border: "1px solid rgba(74,83,67,0.18)" }}>
                    <div className="font-mono text-4xl font-light text-[#1A221E]">{card.value}</div>
                    <div className="mt-1 text-[11px] uppercase tracking-wider text-[#4A5343]/80">{card.title}</div>
                    <div className="text-[10px] text-[#1A221E]/40">{card.sub}</div>
                  </div>
                ))}
              </div>
              {galleryImages.length > 0 && (
                <div className="flex gap-2">
                  {galleryImages.map((src, i) => (
                    <div key={i} className="sd-row relative h-44 flex-1 cursor-pointer overflow-hidden" style={{ border: "1px solid rgba(74,83,67,0.12)" }} onClick={() => setLightboxSrc(src)}>
                      <img src={src} alt="" className="absolute inset-0 h-full w-full object-cover opacity-70 transition-opacity hover:opacity-100" />
                    </div>
                  ))}
                </div>
  ...
  return (
    <>
      ...
      <div ref={rootRef} className="fixed inset-0 overflow-hidden bg-[#8A9A86]">
            ...
            >
              <div
                ...
                style={{
                  ...
                  backgroundColor: i === currentScene ? "#4A5343" : "rgba(26,34,30,0.2)",
                }}
              />
            </button>
          ))}
        ...
        <div className="fixed bottom-8 left-8 z-50 flex items-baseline gap-1.5">
          <span className="text-lg font-light text-[#1A221E]">{String(currentScene + 1).padStart(2, "0")}</span>
          <span className="text-xs text-[#1A221E]/20">/</span>
          <span className="text-xs text-[#1A221E]/30">{String(totalScenes).padStart(2, "0")}</span>
        </div>
      ...
      {selectedProject && (
        <>
          ...
          <button
            ...
            style={{ fontFamily: 'monospace', fontSize: '11px', letterSpacing: '0.2em', color: 'rgba(26,34,30,0.7)', background: 'rgba(36,47,42,0.12)', border: '1px solid rgba(74,83,67,0.35)', padding: '10px 18px' }}
          >
          ...
          >
            ...
            >
              <div ref={modalContentRef} className="bg-[#242F2A]">
              ...
              <div className="modal-hero" style={{ position: 'relative', height: '140vh' }}>
                ...
                />
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, #242F2A 0%, rgba(36,47,42,0.3) 50%, transparent 100%)' }} />
                <div
              ...
              <div className="modal-scene-2 relative" style={{ paddingTop: '100px', paddingBottom: '120px', background: '#E8E0D5' }}>
                ...
                <div className="modal-scene-2-bg absolute inset-0" style={{ background: '#242F2A', opacity: 0 }} />
              ...
              <div className="modal-scene-4" style={{ opacity: 0, background: '#242F2A', minHeight: '100vh', padding: '120px 64px' }}>
      ...
      {showContactModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center px-4 pointer-events-auto">
          ...
          />
          <div className="relative z-10 w-full max-w-lg border border-[#1A221E]/[0.15] bg-[#8A9A86] p-8">
            <button
              type="button"
              onClick={() => { setShowContactModal(false); setContactStatus("idle") }}
              className="absolute right-4 top-4 flex h-10 w-10 cursor-pointer items-center justify-center text-[#1A221E]/55 transition-colors hover:text-[#1A221E]"
            ...
            <form onSubmit={submitContact} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input
                  ...
                  className="border-0 border-b border-[#1A221E]/20 bg-transparent px-0 py-3 text-sm text-[#1A221E] placeholder-[#1A221E]/30 outline-none transition-colors focus:border-[#4A5343]/50"
                />
                <input
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select
                  name="type"
                  className="border-0 border-b border-[#1A221E]/20 bg-transparent px-0 py-3 text-sm text-[#1A221E]/75 outline-none transition-colors focus:border-[#4A5343]/50"
                >
              ...
              <textarea
                ...
                className="resize-none border-0 border-b border-[#1A221E]/20 bg-transparent px-0 py-3 text-sm text-[#1A221E] placeholder-[#1A221E]/30 outline-none transition-colors focus:border-[#4A5343]/50"
              />
              <div className="flex items-center gap-4">
                <button
                  type="submit"
                  disabled={contactStatus === "loading"}
                  className="bg-[#4A5343] px-8 py-4 text-[11px] uppercase tracking-[0.2em] text-[#1A221E] transition-colors hover:bg-[#242F2A] disabled:opacity-60"
                >
